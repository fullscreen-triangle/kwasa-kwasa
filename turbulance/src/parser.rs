...
use crate::ast::{Node, BinaryOp, UnaryOp, TextOp, Parameter, Motion, Source, Span, Position};
...
impl Parser {
    ...
    fn project_declaration(&mut self) -> Result<Node> {
        ...
        let attributes = if self.match_token(&TokenKind::LeftParen) {
            ...
            self.consume(&TokenKind::RightParen, "Expected ')' after project attributes")?;
            attrs
        } else {
            HashMap::new()
        };
    ...
    fn proposition_declaration(&mut self) -> Result<Node> {
        ...
        let body_span = if let Some(last_stmt) = statements.last() {
            self.span_from_to(&start_pos, last_stmt.span())
        } else {
    ...
    fn point_declaration(&mut self) -> Result<Node> {
        ...
        self.consume(&TokenKind::LeftBrace, "Expected '{' to start point properties")?;
        ...
        self.consume(&TokenKind::RightBrace, "Expected '}' after point properties")?;
    ...
    fn return_statement(&mut self) -> Result<Node> {
        ...
        let span = if let Some(ref val) = value {
            self.span_from_to(&start_pos, val.span())
        } else {
            self.span_from_to(&start_pos, &start_pos)
        };
    ...
    fn given_statement(&mut self) -> Result<Node> {
        ...
        let span = self.span_from_to(&start_pos, end_span);
    ...
    fn primary(&mut self) -> Result<Node> {
        ...
        if self.match_token(&TokenKind::Identifier) {
            let token = self.previous();
            return Ok(Node::identifier(token.lexeme.clone(), self.token_span(token)));
        }
    ...
    fn array_literal(&mut self) -> Result<Node> {
        ...
        let end_pos = self.consume(&TokenKind::RightBracket, "Expected ']' after array elements")?;
    ...
    fn token_span(&self, token: &Token) -> Span {
        Span::new(
            ...
            Position::new(token.line, token.column + token.lexeme.len(), token.span.end),
        )
    }
