...
\title{Circuit Based Cognition Mechanics: Trajectory Completion Dynamics in Psychon Circuits}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig1_sentropy_coordinates.pdf}
\caption{\textbf{S-Entropy Coordinate System: Universal Basis for Categorical States.}
The tri-dimensional S-entropy coordinates $(S_k, S_t, S_e) \in [0,1]^3$ provide a sufficient basis for categorical state specification in bounded systems.
\textbf{Panel (A):} Three-dimensional S-entropy space $\mathcal{S}_N$ showing psychon state distribution (colored points within unit cube). Points are colored by $S_e$ value (viridis colormap). Clustering reveals natural equivalence classes in mental state space. The unit cube boundary (black edges) enforces normalization. Cross-domain equivalence holds when $\|(\mathbf{s}_1 - \mathbf{s}_2)\| < \varepsilon$ with $\varepsilon \approx 0.1$.
\textbf{Panel (B):} Information deficit $S_k = -\log P$ as function of equivalence class probability $P$. High probability states have low information deficit (near origin); rare states have high $S_k$. The red marker at $P = 0.5$ shows $S_k = \ln 2 \approx 0.69$. Blue shaded region indicates the probability-information relationship. This coordinate captures ``how much is known'' about the categorical state.
\textbf{Panel (C):} Temporal position $S_t = n/N$ showing progression through categorical completion. Bar heights indicate fractional progress from initiation ($S_t = 0$) to completion ($S_t = 1$). Green arrow indicates temporal flow direction. This coordinate captures ``how far along'' a process has progressed, independent of clock time.
\textbf{Panel (D):} Categorical entropy $S_e = -\sum p_i \log p_i$ for four different state distributions (colored bars). Peaked distributions (red, $\alpha = 0.5$) have low entropy; uniform distributions (blue, $\alpha = 5.0$) have high entropy. Legend shows computed $S_e$ values. This coordinate captures ``how spread out'' the probability mass is over possible states.}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig2_trajectory_terminus.pdf}
\caption{\textbf{Psychon as Trajectory-Terminus Pair: $\mathcal{M} = (\gamma, \Gamma_f)$.}
Mental states are defined not by endpoint alone but by the trajectory-terminus pair, explaining context-dependence and state non-identity.
\textbf{Panel (A):} Three-dimensional S-entropy space showing four distinct trajectories (blue, green, purple, orange curves) converging to the same terminus state $\Gamma_f$ (red star). Each trajectory originates from different regions (colored circles) but reaches identical final coordinates. Despite terminus identity, these represent four distinct mental states because $\gamma_1 \neq \gamma_2 \Rightarrow \mathcal{M}_1 \neq \mathcal{M}_2$.
\textbf{Panel (B):} Context dependence visualization. Central yellow circle represents identical thought content (``cup on chair''). Four colored arrows indicate different trajectories (contexts): Party (red), Exam (blue), Search (green), Memory (purple). Same semantic content produces four distinct mental states through different approach trajectories. The psychon framework captures what computational approaches miss.
\textbf{Panel (C):} Ternary address structure for trajectory encoding. Each node branches into three children, creating addresses $(a_1, a_2, \ldots, a_k)$ with $a_i \in \{0, 1, 2\}$. Navigation complexity is $O(\log_3 n)$, providing 37\% efficiency over binary addressing. The address IS the path---trajectory-position identity means specifying the address specifies the trajectory.
\textbf{Panel (D):} Mental state non-identity theorem demonstration. Trajectories $\gamma_1$ and $\gamma_2$ differ (steel blue and coral bars), terminus $\Gamma_f$ is identical (both at 0.5), yet mental states $\mathcal{M}_1 \neq \mathcal{M}_2$ (separated bars with $\neq$ symbol). The ordered pair structure $(\gamma, \Gamma_f)$ guarantees non-identity when trajectories differ.}
\label{fig:trajectory_terminus}
\end{figure*}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig3_circuit_equivalence.pdf}
\caption{\textbf{Circuit Equivalence: Simultaneous R-C-L Operation in S-Dimensions.}
Each psychon operates simultaneously as resistor (in $S_k$), capacitor (in $S_t$), and inductor (in $S_e$), with mode selected by S-entropy minimization.
\textbf{Panel (A):} Three-dimensional mode selection surface. Color indicates dominant mode: red regions where $S_k$ dominates (resistive), green where $S_t$ dominates (capacitive), blue where $S_e$ dominates (inductive). The surface partitions S-entropy space into operational regimes. Mode transitions occur at boundaries where two coordinates are equal.
\textbf{Panel (B):} R-C-L trichotomy Venn diagram showing simultaneous operation. Three overlapping circles represent resistive ($S_k$, red), capacitive ($S_t$, green), and inductive ($S_e$, blue) modes. The central overlap region contains ``Psychon''---every psychon exhibits all three behaviors simultaneously. Actual instantaneous mode is selected by context-dependent weighting $(\alpha, \beta, \gamma)$.
\textbf{Panel (C):} S-entropy cost minimization for mode selection. Four contexts shown: High $S_k$ (select R), High $S_t$ (select C), High $S_e$ (select L), Balanced. Grouped bars show cost $= \alpha S_k + \beta S_t + \gamma S_e$ decomposed by mode contribution. Selected mode minimizes total weighted cost. This mechanism enables context-sensitive circuit behavior.
\textbf{Panel (D):} Impedance frequency response on log-log scale. Resistive mode: constant $|Z_R| = R$ (red). Capacitive mode: $|Z_C| = 1/\omega C$ decreasing with frequency (green). Inductive mode: $|Z_L| = \omega L$ increasing with frequency (blue). Purple dashed line marks resonance frequency $\omega_0 = 1/\sqrt{LC}$ where capacitive and inductive impedances cross.}
\label{fig:circuit_equivalence}
\end{figure*}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig4_consciousness_intersection.pdf}
\caption{\textbf{Consciousness as Geometric Intersection: $\mathcal{C}(t) = P_{\text{decay}}(t) \cap T_{\text{decay}}(t)$.}
Consciousness is the geometric intersection of perception and thought decay curves, with magnitude determined by the minimum of the two.
\textbf{Panel (A):} Three-dimensional visualization of decay curve intersection. Blue surface represents perception decay $P_{\text{decay}}$; red surface represents thought decay $T_{\text{decay}}$. Green line traces the intersection $\mathcal{C} = P \cap T$ where both surfaces meet. Consciousness exists only in regions where both surfaces are above threshold---the intersection is literally the conscious state.
\textbf{Panel (B):} Temporal evolution of decay curves. Blue: $P_{\text{decay}}(t)$ driven by sensory input with time constant $\tau_P \approx 50$ ms. Red: $T_{\text{decay}}(t)$ driven by cognitive processes with $\tau_T \approx 100$ ms. Green shaded region shows intersection where both exceed threshold. Consciousness waxes and wanes as decay curves rise and fall with changing inputs.
\textbf{Panel (C):} Consciousness window $W_C = \{t : P > \theta \land T > \theta\}$. Gold shaded regions indicate temporal windows where consciousness is present. Black dashed line marks threshold $\theta$. Decay curves (blue, red) must both exceed threshold simultaneously. Window duration follows $\Delta t_C = \tau_P \tau_T / (\tau_P + \tau_T) \cdot \ln(P_0 T_0 / \theta^2)$, typically 100--500 ms.
\textbf{Panel (D):} Consciousness magnitude $|\mathcal{C}| = \min(P_{\text{decay}}, T_{\text{decay}})$. Dashed curves show individual decay functions; solid green shows the minimum (consciousness magnitude). Green shading indicates integrated conscious experience. Consciousness is limited by the weaker of perception or thought at each moment---strengthening one without the other cannot increase $|\mathcal{C}|$.}
\label{fig:consciousness_intersection}
\end{figure*}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig5_emergent_phenomena.pdf}
\caption{\textbf{Emergent Mental Phenomena: Memory, Dreams, and Pharmacological Effects.}
Memory formation, dream dynamics, and drug effects emerge naturally from the trajectory-terminus framework.
\textbf{Panel (A):} Three-dimensional memory accumulation. Blue curve shows trajectory through $(t, H(t), M(t))$ space where $H$ is emotional field and $M = \int \dot{H}^+ dt$ is accumulated memory. Red dashed line shows emotional field evolution at $M = 0$ plane. Red markers indicate high emotional change events ($\dot{H}^+ > 0.1$) that contribute strongly to memory formation. Memory encoding requires emotional dynamics, not static states.
\textbf{Panel (B):} Dream dynamics characterized by $P_{\text{decay}} = 0$, $T_{\text{decay}} > 0$. Solid curves show waking state (both $P$ and $T$ active); dashed curves show dream state where perception vanishes but thought continues. Purple shaded region marks dream period. Without perceptual constraint ($P = 0$), thought trajectories evolve freely---explaining dream phenomenology: unconstrained association, emotional intensity, time distortion.
\textbf{Panel (C):} Pharmacological trajectory modification: $\mathcal{P}_{\text{drug}}: \gamma \to \gamma'$. Red curve shows anxious trajectory (high variance, spiky); green shows calm trajectory (smooth). Both reach the same terminus (black stars)---thought content is identical. Yet mental states differ because trajectories differ. Anxiolytics modify the path, not the destination, changing experience without changing semantic content.
\textbf{Panel (D):} Mental state equivalence classes $[\mathcal{M}]$ visualized as clusters in $(S_k, S_t)$ projection. Four colored clusters represent distinct equivalence classes, each containing states with similar trajectories and termini. Dashed ellipses indicate class boundaries defined by $|\text{Context}(\gamma_1) - \text{Context}(\gamma_2)| < \varepsilon$. States within a class are phenomenologically similar; states across classes are distinct.}
\label{fig:emergent_phenomena}
\end{figure*}
