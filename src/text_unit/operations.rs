...
use crate::text_unit::{TextUnit, TextUnitRegistry, TextUnitType};
use rand;
use regex;
use std::collections::HashMap;

/// Placeholder for AST Value type since turbulance module is not available
#[derive(Debug, Clone)]
pub enum Value {
    String(String),
    List(Vec<Value>),
...
impl OperationResult {
        ...
        match self {
            ...
            OperationResult::Units(units) => {
                ...
                let unit = TextUnit::new(
                    ...
                    TextUnitType::Custom("merged".to_string()), // Merged unit
                    0,                                          // hierarchy_level
                );
            ...
            OperationResult::Value(Value::TextUnit(ast_unit)) => {
                let mut unit = TextUnit::new(
                    ...
                    0, // Position unknown
                    0, // Position unknown
                    TextUnitType::Custom("ast_unit".to_string()),
                    0, // hierarchy_level
                );

                // Copy metadata from ast_unit if available
                if let Some(metadata) = ast_unit.metadata.get("metadata") {
                    unit.metadata
                        .insert("original_metadata".to_string(), metadata.clone());
                }
    ...
    pub fn to_value(self) -> Result<Value, String> {
        match self {
            OperationResult::Unit(unit) => Ok(Value::TextUnit(unit)),
            OperationResult::Units(units) => {
                let ast_units = units.into_iter().map(|u| Value::TextUnit(u)).collect();
...
pub fn divide(
    ...
    if unit_type_level(unit.unit_type.clone()) <= unit_type_level(division_type.clone()) {
        return OperationResult::Error(format!(
            "Cannot divide a {:?} into {:?}s",
            unit.unit_type, division_type
        ));
    }

    // Get child units of the target type if they already exist
    let existing_children = registry
        .get_children(unit.id)
        .into_iter()
    ...
    match division_type {
        ...
        TextUnitType::Word => {
            // Split into words
            let words = unit.content.split_whitespace().collect::<Vec<&str>>();
...
pub fn multiply(
    ...
    sorted_units.sort_by_key(|u| u.start_pos);

    // Check that all units are of the same type
    let first_type = sorted_units[0].unit_type.clone();
    ...
    if unit_type_level(first_type.clone()) >= unit_type_level(result_type.clone()) {
        return OperationResult::Error(format!(
    ...
    let start = sorted_units.first().unwrap().start_pos;
    let end = sorted_units.last().unwrap().end_pos;

    let mut result_unit = TextUnit::new(
        combined,
        start,
        end,
        result_type,
    ...
    // Add metadata
    result_unit.metadata = metadata;

    OperationResult::Unit(result_unit)
}

/// Add two text units (concatenation)
pub fn add(unit1: &TextUnit, unit2: &TextUnit, registry: &mut TextUnitRegistry) -> OperationResult {
    // Determines which unit comes first
    let (first, second) = if unit1.start_pos <= unit2.start_pos {
        (unit1, unit2)
    } else {
        (unit2, unit1)
    };
    ...
    let mut result_unit = TextUnit::new(
        ...
        first.start_pos,
        second.end_pos,
        first.unit_type.clone(), // Keep the same type
        0,                       // hierarchy_level
    );
    result_unit.metadata = metadata;
...
pub fn subtract(
    ...
    let mut result_unit = TextUnit::new(
        ...
        from.start_pos + new_content.len(),
        from.unit_type.clone(),
        0, // hierarchy_level
    );
    result_unit.metadata = from.metadata.clone();
...
pub fn transform(
    ...
    let transformed = match transformation {
        "simplify" => {
            ...
            match level {
                ...
                _ => {
                    return OperationResult::Error(format!(
                        "Unsupported simplification level: {}",
                        level
                    ));
                }
            }
    ...
    let mut result_unit = TextUnit::new(
        transformed.clone(),
        unit.start_pos,
        unit.start_pos + transformed.len(),
        unit.unit_type.clone(),
        0, // hierarchy_level
    );
    result_unit.metadata = unit.metadata.clone();
...
pub fn pipeline(
    ...
    for (op, args) in operations {
        current_result = match current_result {
            OperationResult::Unit(unit) => match *op {
                "filter" => {
            ...
            _ => {
                return OperationResult::Error(
                    "Previous operation didn't produce a valid unit".to_string(),
                );
            }
        };
    }
...
pub fn compose(
    ...
    for i in 0..sorted_units.len() {
        if i > 0 {
            ...
            let transition = match style {
                "sequential" => match sorted_units[0].unit_type {
                    TextUnitType::Sentence => " Then, ",
                    TextUnitType::Paragraph => "\n\nNext, ",
                    _ => "\n\nFollowing this, ",
                },
                "causal" => match sorted_units[0].unit_type {
                    TextUnitType::Sentence => " Therefore, ",
                    TextUnitType::Paragraph => "\n\nAs a result, ",
                    _ => "\n\nConsequently, ",
                },
                "contrastive" => match sorted_units[0].unit_type {
                    TextUnitType::Sentence => " However, ",
                    TextUnitType::Paragraph => "\n\nIn contrast, ",
                    _ => "\n\nOn the other hand, ",
                },
                "elaborative" => match sorted_units[0].unit_type {
                ...
                _ => match sorted_units[0].unit_type {
                    TextUnitType::Sentence => " ",
                    TextUnitType::Paragraph => "\n\n",
                    _ => "\n\n",
                },
    ...
    let result_unit = TextUnit::new(
        composed.clone(),
        sorted_units[0].start_pos,
        sorted_units[0].start_pos + composed.len(),
        sorted_units[0].unit_type.clone(),
        0, // hierarchy_level
    );
...
mod tests {
    ...
    fn test_pipeline_operations() {
        ...
        filter_args.insert(
            "predicate".to_string(),
            Value::String("contains(LONG)".to_string()),
        );
    ...
    fn test_compose_text_units() {
        ...
        match result {
            OperationResult::Unit(composed) => {
                assert_eq!(
                    composed.content,
                    "This is the first point However, This is the second point"
                );
            }
