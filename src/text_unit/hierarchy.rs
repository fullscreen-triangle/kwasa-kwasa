...
use crate::text_unit::boundary::{
    detect_paragraph_boundaries, detect_sentence_boundaries, detect_word_boundaries,
};
use crate::text_unit::types::{BoundaryType, TextUnit};
...
impl DocumentHierarchy {
    ...
    pub fn from_text_unit(text_unit: TextUnit) -> Self {
        ...
        // Clone the text unit content for processing
        let text_content = text_unit.content.clone();

        let root = HierarchyNode::new(node_type, text_unit, root_id, None);
        hierarchy.root = Some(root);

        hierarchy.process_children(text_content);

        hierarchy
    }

    /// Process children of a text unit and add them to the hierarchy
    fn process_children(&mut self, text_content: String) {
        if let Some(root) = &mut self.root {
            // First process paragraphs
            if root.node_type() == &NodeType::Document {
                self.extract_and_add_children(&text_content, root.id(), BoundaryType::Structural);
            }

            // Then process sentences within paragraphs
            let paragraph_ids: Vec<usize> = self
                .find_nodes_by_type(&NodeType::Paragraph)
                .iter()
                .map(|p| p.id())
                .collect();
            for paragraph_id in paragraph_ids {
                if let Some(paragraph) = self.find_node_by_id(paragraph_id) {
                    let content = paragraph.content().content.clone();
                    self.extract_and_add_children(&content, paragraph_id, BoundaryType::Structural);
                }
            }
            ...
            let sentence_ids: Vec<usize> = self
                .find_nodes_by_type(&NodeType::Sentence)
                .iter()
                .map(|s| s.id())
                .collect();
            for sentence_id in sentence_ids {
                if let Some(sentence) = self.find_node_by_id(sentence_id) {
                    let content = sentence.content().content.clone();
                    self.extract_and_add_children(&content, sentence_id, BoundaryType::Structural);
                }
            }
        }
    }
    ...
    fn extract_and_add_children(
        ...
        let child_ids =
            detect_paragraph_boundaries(parent_content, &mut registry, &Default::default());
        ...
        for child_id in child_ids {
            if let Some(child_unit) = registry.get_unit(child_id) {
                ...
                let node =
                    HierarchyNode::new(node_type, child_unit.clone(), node_id, Some(parent_id));
    ...
    pub fn find_nodes_by_type(&self, node_type: &NodeType) -> Vec<&HierarchyNode> {
        self.root
            .as_ref()
            .map_or_else(Vec::new, |root| root.find_by_type(node_type))
    }
    ...
    pub fn find_nodes_by_content(&self, substring: &str) -> Vec<&HierarchyNode> {
        self.root
            .as_ref()
            .map_or_else(Vec::new, |root| root.find_by_content(substring))
    }
    ...
    pub fn text(&self) -> String {
        self.root
            .as_ref()
            .map_or_else(String::new, |root| root.text().to_string())
    }
    ...
    pub fn nodes_at_level(&self, level: usize) -> Vec<&HierarchyNode> {
        self.root
            .as_ref()
            .map_or_else(Vec::new, |root| root.nodes_at_depth(level))
    }
    ...
    pub fn build_semantic_hierarchy(&self) -> Self {
        ...
        if let Some(root) = &self.root {
            let new_root = HierarchyNode::new(NodeType::Document, root.content().clone(), 0, None);
            ...
            if !sentences.is_empty() {
                ...
                for s in 0..sentences.len() {
                    ...
                    let keywords: HashSet<String> = sentence
                        .text()
                        .split_whitespace()
    ...
    fn add_semantic_block(&mut self, sentences: &[&HierarchyNode]) {
        if let Some(root) = &mut self.root {
            ...
            let mut block_node =
                HierarchyNode::new(NodeType::SemanticBlock, text_unit, node_id, Some(root.id()));
    ...
    fn depth_first_visit<'a>(
        &'a self,
        node: &'a HierarchyNode,
        result: &mut Vec<&'a HierarchyNode>,
    ) {
        result.push(node);
...
mod tests {
    use super::*;
    use crate::text_unit::types::BoundaryType;

    /// Helper to create a test text unit
    fn create_test_unit(content: &str, unit_type: crate::text_unit::TextUnitType) -> TextUnit {
        TextUnit::new(content.to_string(), 0, content.len(), unit_type, 0)
    }

    #[test]
    fn test_hierarchy_node_creation() {
        let unit = create_test_unit("Test content", crate::text_unit::TextUnitType::Document);
    ...
    fn test_document_hierarchy_creation() {
        ...
        assert!(hierarchy.count_nodes() > 0); // Should have created at least the root node
    }
    ...
    fn test_hierarchy_navigation() {
        ...
        let doc_unit = create_test_unit("Document", crate::text_unit::TextUnitType::Document);
        let mut root = HierarchyNode::new(NodeType::Document, doc_unit, 0, None);

        let para1_unit = create_test_unit("Paragraph 1", crate::text_unit::TextUnitType::Paragraph);
        let mut para1 = HierarchyNode::new(NodeType::Paragraph, para1_unit, 1, Some(0));

        let para2_unit = create_test_unit("Paragraph 2", crate::text_unit::TextUnitType::Paragraph);
        let para2 = HierarchyNode::new(NodeType::Paragraph, para2_unit, 2, Some(0));

        let sent1_unit = create_test_unit("Sentence 1.", crate::text_unit::TextUnitType::Sentence);
        let sent1 = HierarchyNode::new(NodeType::Sentence, sent1_unit, 3, Some(1));

        let sent2_unit = create_test_unit("Sentence 2.", crate::text_unit::TextUnitType::Sentence);
    ...
    fn test_semantic_hierarchy_building() {
        ...
        let unit = create_test_unit(text, crate::text_unit::TextUnitType::Document);
        ...
        assert!(
            semantic_hierarchy
                .find_nodes_by_type(&NodeType::SemanticBlock)
                .len()
                >= 0
        );

        // The hierarchy should contain the document
        assert!(semantic_hierarchy.count_nodes() >= 1);
    }
}
