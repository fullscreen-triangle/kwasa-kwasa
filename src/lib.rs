...
pub use error::{Error, ErrorReporter};
...
pub use orchestrator::{
    ActiveIntervention, Config as OrchestratorConfig, Context, Goal, GoalMetrics, GoalStatus,
    GoalType, InterventionOutcome, InterventionSystem, InterventionType, Strategy,
    SuccessCriterion, WritingPattern,
};
...
impl KwasaFramework {
    ...
    pub async fn new(config: FrameworkConfig) -> FrameworkResult<Self> {
        ...
        info!(
            "Initializing Kwasa-Kwasa Framework with session ID: {}",
            session_id
        );
        ...
        // Create default goal
        let default_goal = Goal::new("General text processing", 0.7);
    ...
    pub async fn process_text(
    ...
    ) -> FrameworkResult<ProcessingResult> {
        self.state = FrameworkState::Processing;
        ...
        let text_unit = TextUnit::new(text.to_string(), 0, text.len(), TextUnitType::Paragraph, 0);
        ...
        let interventions = self
            .get_intervention_suggestions(text, &quality_metrics)
            .await?;
    ...
    pub async fn set_goal(
        ...
        let goal = Goal::new(description, priority.unwrap_or(0.7));
    ...
    async fn get_intervention_suggestions(
        &self,
        text: &str,
        quality: &QualityMetrics,
    ) -> FrameworkResult<Vec<InterventionSuggestion>> {
