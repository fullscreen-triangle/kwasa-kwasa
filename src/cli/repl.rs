...
use crate::error::{Error, ErrorReporter, Result};
use crate::orchestrator::Context;
// use crate::turbulance; // Commented out - module not available
...
impl Completer for ReplHelper {
    ...
    fn complete(
        ...
        if line.starts_with(':') {
            let commands = vec![
                ":exit", ":help", ":history", ":clear", ":reset", ":load ", ":save ", ":run",
                ":debug",
            ];
        ...
        if pos > word_start {
            ...
            // Complete keywords
            let keyword_matches: Vec<Pair> = self
                .keywords
                .iter()
...
impl Repl {
    ...
    pub fn new() -> Result<Self> {
        ...
        let home_dir =
            dirs::home_dir().ok_or_else(|| Error::cli("Could not determine home directory"))?;
    ...
    fn print_welcome_message(&self) {
        println!("{} v{}", "Turbulance REPL".green().bold(), "1.0.0");
        println!(
    ...
    fn list_directory(&self) -> Result<bool> {
        let dir_path = match &self.working_directory {
            ...
            None => std::env::current_dir()
                .map_err(|e| Error::cli(format!("Failed to get current directory: {}", e)))?,
        };
        ...
        for entry in entries {
            ...
            let metadata = entry
                .metadata()
                .map_err(|e| Error::cli(format!("Failed to read metadata: {}", e)))?;
