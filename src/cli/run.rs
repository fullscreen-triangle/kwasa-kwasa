...
use crate::cli::config::KwasaConfig;
use crate::error::KwasaError;
...
impl EnvironmentManager {
    ...
    pub fn initialize(workspace_path: &Path) -> Result<Self, KwasaError> {
        ...
        let manifest_content = toml::to_string_pretty(&environment_manifest)
            .map_err(|e| KwasaError::ConfigError(format!("Failed to serialize manifest: {}", e)))?;
        fs::write(&manifest_path, manifest_content)
            .map_err(|e| KwasaError::IoError(format!("Failed to write manifest: {}", e)))?;
    ...
    fn create_workspace_structure(workspace_root: &Path) -> Result<(), KwasaError> {
        let directories = [
        ...
        for dir in &directories {
            ...
            fs::create_dir_all(&dir_path)
                .map_err(|e| KwasaError::IoError(format!("Failed to create directory {}: {}", dir, e)))?;
        }
    ...
    /// Create initial configuration files
    fn create_initial_configs(workspace_root: &Path) -> Result<(), KwasaError> {
...
[dependencies]
{}
"#, self.environment_manifest.dependencies.rust_crates
    .iter()
    ...
    .join("\n"));
        ...
        fs::create_dir_all(&rust_env_path)
            .map_err(|e| KwasaError::IoError(format!("Failed to create Rust environment: {}", e)))?;
...
"#;
    ...
    fn setup_python_environment(&self) -> Result<(), KwasaError> {
        ...
        let output = Command::new("python3")
            ...
            .output()
            .map_err(|e| KwasaError::EnvironmentError(format!("Failed to create Python venv: {}", e)))?;
        
        if !output.status.success() {
            return Err(KwasaError::EnvironmentError(
                format!("Python venv creation failed: {}", String::from_utf8_lossy(&output.stderr))
            ));
        }
        
        // Create requirements.txt
        let requirements_content = self.environment_manifest.dependencies.python_packages.join("\n");
        fs::write(python_env_path.join("requirements.txt"), requirements_content)
...
"#, self.environment_manifest.dependencies.r_packages
    .iter()
    ...
    fn install_scientific_tools(&self) -> Result<(), KwasaError> {
        ...
        for tool in &self.environment_manifest.tools {
            ...
            if Command::new("sh").args(&["-c", &tool.verification_command]).output().is_ok() {
    ...
    fn setup_scientific_databases(&self) -> Result<(), KwasaError> {
        ...
        fs::create_dir_all(&db_path)
            .map_err(|e| KwasaError::IoError(format!("Failed to create database directory: {}", e)))?;
...
databases:
{}
"#, self.environment_manifest.dependencies.scientific_databases
    .iter()
    .map(|db| format!("  - name: {}\n    url: https://{}.org\n    local_cache: true", db, db))
    .collect::<Vec<_>>()
    .join("\n"));
    ...
    fn check_tools_status(&self) -> Vec<(String, bool)> {
        self.environment_manifest.tools
            .iter()
            .map(|tool| {
...
impl EnvironmentStatus {
    pub fn print_status(&self) {
        ...
        println!("Python: {}", if self.python_available { "✅" } else { "❌" });
