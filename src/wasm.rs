...
impl KwasaWasm {
    ...
    pub fn new(config: KwasaConfig) -> Self {
        ...
        info!(
            "KwasaWasm initialized with goal: {}",
    ...
    pub fn execute_code(&mut self, code: String) -> JsValue {
        ...
        match tokens {
            Ok(tokens) => {
                ...
                match parser.parse() {
                    Ok(ast) => match self.interpreter.evaluate(&ast) {
                        Ok(value) => {
    ...
    pub fn divide_text(&self, text: String, unit_type: String) -> JsValue {
        ...
        let document_unit = TextUnit::new(text.clone(), 0, text.len(), TextUnitType::Document, 0);

        // Divide into the requested units
        let units = document_unit.divide(unit_type);

        // Convert to JS-friendly format
        let result: Vec<String> = units.into_iter().map(|unit| unit.content).collect();
